function RadarChart(config = null) {
	...
	this.paddingX = config.paddingX || 20
	this.paddingY = config.paddingY || 25

	this.labelColor = config.labelColor || "#212121"
	this.labelSize = config.labelSize || 10
	this.labelPadding = config.labelPadding || 20

	this.axesColor = config.axesColor || "#ee9ddc"
	this.axesWidth = config.axesWidth || 1

	this.lineColor = config.lineColor || "#c239a3"
	this.lineFillColor = config.lineFillColor || "#c239a320"
...
RadarChart.prototype.Plot = function(svg, data) {
	...
	let x = size / 2 + 2 * this.paddingX // Math.floor(width / 2)//
